...
const CategoriesPage: React.FC = () => {
  ...
  const createCategoryMutation = useMutation({
    mutationFn: (newCategory: CategoryInput) => apiClient.categoriesService.categoriesCreate(newCategory),
    onSuccess: () => {
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (id: string) => apiClient.categoriesService.categoriesDelete({ id }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['categories'] });
    },
  });
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
        {categories?.data?.map((category) => (
          ...
          >
              ...
              >
                {category.icon && (
                  <span style={{ color: 'white', fontSize: '1.2em' }}>
                    {category.icon === 'microfrontend-icon' ? '⚛️' : 
            ...
            <p style={{ margin: '0 0 15px 0', color: '#666' }}>{category.description}</p>

            {/* Category stats */}
            <div style={{ marginBottom: '15px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                <span style={{ fontSize: '0.9em', color: '#666' }}>Product Count:</span>
                <span style={{
                  backgroundColor: category.color,
                  color: 'white',
                  padding: '2px 8px',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontSize: '0.9em', color: '#666' }}>Sort Order:</span>
                <span style={{ fontSize: '0.9em', fontWeight: 'bold' }}>{category.sortOrder}</span>
              </div>
            </div>
