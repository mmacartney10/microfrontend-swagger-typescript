...
const OrdersPage: React.FC = () => {
  ...
  const { data: selectedOrder, isLoading: loadingOrder } = useQuery({
    ...
    queryFn: () => apiClient.ordersService.ordersDetail({ id: selectedOrderId! }),
    enabled: !!selectedOrderId,
  });

  // Create order mutation
  const createOrderMutation = useMutation({
    mutationFn: (newOrder: OrderInput) => apiClient.ordersService.ordersCreate(newOrder),
    onSuccess: () => {
            ...
            loadingOrder ? (
              ...
              <div style={{
                ...
                <div style={{ marginBottom: '15px' }}>
                  ...
                  <span style={{
                    backgroundColor: getStatusColor(selectedOrder.data.status),
                    color: 'white',
                    padding: '6px 12px',
                    borderRadius: '8px',
                    textTransform: 'capitalize'
                ...
                <div style={{ marginBottom: '15px' }}>
                  ...
                  <p><strong>Name:</strong> {selectedOrder.data.customerName}</p>
                  <p><strong>ID:</strong> {selectedOrder.data.customerId}</p>
                  <p><strong>Shipping Address:</strong> {selectedOrder.data.shippingAddress}</p>
                </div>
                ...
                <div style={{ marginBottom: '15px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.2em', fontWeight: 'bold' }}>
                    <span>Total:</span>
                    <span style={{ color: '#8B5CF6' }}>${selectedOrder.data.total}</span>
                  </div>
                </div>

                <div style={{ fontSize: '0.9em', color: '#666' }}>
                  {selectedOrder.data.orderDate && (
                    <p>Order Date: {new Date(selectedOrder.data.orderDate).toLocaleString()}</p>
                  )}
                  {selectedOrder.data.estimatedDelivery && (
                    <p>Estimated Delivery: {new Date(selectedOrder.data.estimatedDelivery).toLocaleDateString()}</p>
                  )}
                </div>
              </div>
            ) : null
