...
const HealthCheckDashboard: React.FC = () => {
  ...
  const checkingCount = healthChecks.filter(
    (h) => h.status === "checking",
  ).length;

  const overallStatus =
    errorCount > 0 ? "degraded" : checkingCount > 0 ? "checking" : "healthy";
  ...
  const StatusIndicator: React.FC<{ status: string; size?: number }> = ({
    status,
    size = 12,
  }) => {
    const color =
      ...
      >
        ...
        >
          ...
          <div style={{ color: errorCount === 0 ? "#10B981" : "#EF4444" }}>
            ...
            {((healthyCount / healthChecks.length) * 100).toFixed(1)}%
            <br />
            <small>
              {errorCount === 0
                ? "All services operational"
                : `${errorCount} service(s) need attention`}
            </small>
          </div>
        </div>
      </div>
