...
const UsersPage: React.FC = () => {
  ...
  const createUserMutation = useMutation({
    mutationFn: (newUser: UserInput) => apiClient.usersService.usersCreate(newUser),
    onSuccess: () => {
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
              <div>
                ...
                <p style={{ margin: '0', color: '#666', fontSize: '0.9em' }}>@{user.username}</p>
              </div>
              <span style={{
            ...
            {user.preferences && (
              <div style={{ marginBottom: '15px', fontSize: '0.9em' }}>
                <h4 style={{ margin: '0 0 10px 0', fontSize: '1em' }}>Preferences</h4>
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  <span style={{
                    backgroundColor: user.preferences.theme === 'dark' ? '#374151' : '#F3F4F6',
                    color: user.preferences.theme === 'dark' ? 'white' : '#374151',
                    padding: '2px 6px',
                    borderRadius: '8px',
                    fontSize: '0.8em'
                  }}>
                  ...
                  <span style={{
                    backgroundColor: user.preferences.notifications ? '#10B981' : '#EF4444',
                    color: 'white',
                    padding: '2px 6px',
                    borderRadius: '8px',
                    fontSize: '0.8em'
