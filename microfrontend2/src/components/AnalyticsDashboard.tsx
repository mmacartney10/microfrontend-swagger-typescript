...
const AnalyticsDashboard: React.FC = () => {
  ...
  const completedTasks =
    tasks?.data?.filter((task) => task.completed)?.length || 0;
  const taskCompletionRate =
    totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;

  const totalProducts = products?.data?.length || 0;
  const activeProducts =
    products?.data?.filter((product) => product.active)?.length || 0;
  ...
  const activeCategories =
    categories?.data?.filter((category) => category.active)?.length || 0;
          ...
          {categoryStats
            ...
            .map((category) => {
              const maxProducts = Math.max(
                ...categoryStats.map((c) => c.productCount),
              );
              const percentage =
                maxProducts > 0
                  ? Math.round((category.productCount / maxProducts) * 100)
                  : 0;
      ...
      >
        ...
        >
          <div>
            ...
            <p>
              You have {totalTasks - completedTasks} pending tasks out of{" "}
              {totalTasks} total.
            </p>
          </div>
          <div>
            ...
            <p>
              {totalProducts - activeProducts} products need attention
              (inactive).
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p>
              {Math.round((activeUsers / totalUsers) * 100)}% of users are
              currently active.
            </p>
          </div>
        </div>
